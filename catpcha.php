...
<body>
    ...
    function captcha($str)
    {
        ...
        for ($i = 0; $i < mb_strlen($gstr); $i++) {
            ...
            // bbox盒子的寬度，imagettfbbox()出來的結果是一個陣列
        ...
        //使用迴圈把驗證碼文字逐一寫入到圖片中[字串本身是陣列，所以可以直接用foreach]
        foreach ($text_info as $char => $info) {

            //計算放置的y坐標範圍，字元的高度加上邊框起始點(5)及總高度-底部坐標終點的限制(5)
            // 讓Ｙ軸的字根據隨機的高度，中間有些許間隔，讓文字可以有不同高度
            ...
            // 讓下一個橫軸的點+1，避免與前一個點的文字重疊
            $x_pointer = $x_pointer + $info['width'] + $info['x'] + 1;
        }

        //建立一個線條範圍亂數，決定圖形驗證碼上的干擾線數量［線要在文字上面］
        $lines = rand(3, 6);
